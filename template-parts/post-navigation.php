        ...
        <?php if ($previou_post) : ?>
            <div class="nav-previous">
                ...
                <a href="<?php the_permalink($previou_post->ID); ?>" aria-label="<?php the_title($previou_post->ID); ?>" class="cover--link">
                    <img src="<?php echo hera_get_background_image($previou_post->ID, 400, 120); ?>" class="cover" alt="<?php the_title($previou_post->ID); ?>" />
        ...
        <?php if ($next_post) : ?>
            <div class="nav-next">
                ...
                <a href="<?php the_permalink($next_post->ID); ?>" aria-label="<?php the_title($next_post->ID); ?>" class="cover--link">
                    <img src="<?php echo hera_get_background_image($next_post->ID, 400, 120); ?>" class="cover" alt="<?php the_title($next_post->ID); ?>" />
                    <?php do_action('marker_pro_post_meta'); ?>
                </a>
            </div>
        <?php endif ?>

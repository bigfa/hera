...
class heraComment
{
    ...
    function handle_archive_view($request)
    {
        ...
        $views = (int)get_term_meta($request['id'], HERA_ARCHIVE_VIEW_KEY, true);
        $views++;
    ...
    function hack_get_comment_text($comment_text, $comment)
    {
        if (!is_comment_feed() && $comment->comment_parent) {
            ...
            if ($parent) {
                ...
                $comment_text = '<a href="' . $parent_link . '" class="hComment--parent">@' . $name . '</a>' . $comment_text;
            }
        }
        return $comment_text;
    }
    ...
    {
        $post_id = $data['id'];
        $post_views = (int)get_post_meta($post_id, HERA_POST_VIEW_KEY, true);
        $post_views++;
    ...
    {
        $post_id = $request['id'];
        $post_views = (int)get_post_meta($post_id, HERA_POST_LIKE_KEY, true);
        $post_views++;
...
?>
        ...
        default:
            ...
            ?>
            <li <?php comment_class('hComment--item') ?> itemtype="http://schema.org/Comment" data-id="<?php comment_ID() ?>" itemscope="" itemprop="comment" id="comment-<?php comment_ID() ?>">
                    ...
                    <header class="hComment--header">
                        ...
                        <div class="hComment--meta">
                            ...
                            <?php if ($comment->user_id == $post->post_author && $heraSetting->get_setting('show_author') &&  !is_admin()) : ?>
                                <svg aria-label="<?php _e('Post Author', 'Hera'); ?>" role="img" viewBox="0 0 40 40" class="author--icon">
                                    <title><?php _e('Post Author', 'Hera'); ?></title>
                            ...
                            <div class="hComment--time" itemprop="datePublished" datetime="<?php echo get_comment_date('c'); ?>"><?php echo human_time_diff(get_comment_time('U'), current_time('U')) .  __(' ago', 'Hera'); ?></div>
                        </div>
                    </header>
